...
from witsense_ugv_rl.startup import startup
import argparse
parser = argparse.ArgumentParser(description="Play a policy in Witsense UGV.")
...
from witsense_ugv_rl.configs import RunConfig
from witsense_ugv_rl.utils import ClipAction
...
if FROM_RUN: # Load paths for run folder
    ...
    run_cfg: RunConfig = pickle.load(open(path_to_run_cfg_pkl, "rb")) # load_yaml does not work on slices
    ...
    # Get policy path
    # get_checkpoint_path feeds this to re.match, so it must be a regex STRING.
    # --checkpoint is typed int, and passing the bare int raises
    # "first argument must be string or compiled pattern". Same shape train_rl.py builds.
    chkpt = rf"model_{args_cli.checkpoint}\.pt" if args_cli.checkpoint is not None else "model_.*"
...
def main(env_cfg: ManagerBasedRLEnvCfg, agent_cfg): # TODO: Add SB3 config support
    ...
    # ClipAction is a gymnasium wrapper, so it must go on BEFORE RslRlVecEnvWrapper —
    # gymnasium asserts its target is a gymnasium.Env. Same order as train_rl.py, which
    # also matters for correctness: the policy must be clipped the way it was trained.
