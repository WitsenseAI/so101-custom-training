...
parser.add_argument('-r', "--run-config-name", type=str, default="RSS_ELEV_CONFIG", help="Run config name.")
...
from witsense_ugv_rl import WITSENSE_UGV_RL_LOGS_DIR

from witsense_ugv_rl.utils import (
...
def main(run_cfg: RunConfig): # TODO: Add SB3 config support
    ...
    if not log_cfg.no_log:
        ...
        with open(os.path.join(log_cfg.run_log_dir, "run_config.pkl"), "wb") as f:
    ...
    if train_cfg.load_run:
        # Exact match, not a prefix: "model_100.*" also matches model_1000.pt, and the
        # helper's sort then returns the longer name.
        chkpt = rf"model_{train_cfg.load_run_checkpoint}\.pt" if train_cfg.load_run_checkpoint > 0 else "model_.*"
        # get path to previous checkpoint
        resume_path = get_checkpoint_path(WITSENSE_UGV_RL_LOGS_DIR, run_dir=train_cfg.load_run,
